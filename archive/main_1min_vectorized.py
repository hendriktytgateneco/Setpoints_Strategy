...
def fetch_minutely_imbalance(start_datetime, end_datetime):
    """
    Fetches 1-min imbalance data from Elia ODS133 API, splitting the request into multiple sub-intervals if needed
    to avoid exceeding the 10,000 row API limit per call. Returns a DataFrame covering the full requested period.
    """
    # Split the period into chunks to avoid API row limits (max 10,000 rows per call)
    chunk_minutes = 7000  # Use 7000 for safety (less than 10,080 in a week)
    all_records = []
...
def simulate_vectorized_setpoints(
...
def optimize_battery_with_delay_vectorized(
    ...
    for charge_setpoint in charge_price_range:
        for discharge_setpoint in discharge_price_range:
            if discharge_setpoint <= charge_setpoint:
                continue
            delivered_kwh, socs, actions, abs_energy_throughput = simulate_vectorized_setpoints(
                imbalance_prices, battery_power_kw, battery_capacity_kwh, soc_init, delay_min, charge_setpoint, discharge_setpoint
            )
...
if __name__ == "__main__":
    ...
    optimize_battery_with_delay_vectorized(start_str, end_str)
